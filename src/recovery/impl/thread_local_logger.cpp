...
std::atomic<size_t> ThreadLocalLogger::ThreadLocalStorageNode::ThreadIdCounter = {0};

ThreadLocalLogger::ThreadLocalLogger(const Config& config) : WorkingDir(config.work_dir) {
...
void ThreadLocalLogger::Enqueue(const WriteSetType& ws_ref, EpochNumber epoch, bool entrusting) {
  if (ws_ref.empty())
    return;
  ...
  if (entrusting) {
    ...
    if (!my_storage->log_file.is_open()) {
      my_storage->log_file =
          std::fstream(GetLogFileName(my_storage->thread_id),
...
void ThreadLocalLogger::TruncateLogs(const EpochNumber checkpoint_completed_epoch) {
  auto* my_storage = thread_key_storage_.Get();

  assert(my_storage->truncated_epoch <= checkpoint_completed_epoch);
  if (checkpoint_completed_epoch == my_storage->truncated_epoch)
    return;
  auto log_filename = GetLogFileName(my_storage->thread_id);
  std::ifstream old_file(log_filename, std::ifstream::in | std::ifstream::binary);

  std::string buffer((std::istreambuf_iterator<char>(old_file)), std::istreambuf_iterator<char>());
  if (buffer.empty()) {
  ...
  for (;;) {
    ...
    try {
      ...
      SPDLOG_ERROR("  Stop recovery procedure: msgpack deserialize failure. Some "
    ...
    deserialized_records.erase(
        remove_if(deserialized_records.begin(), deserialized_records.end(),
                  [&](auto record) { return record.epoch < checkpoint_completed_epoch; }),
        deserialized_records.end());
    records.insert(records.end(), deserialized_records.begin(), deserialized_records.end());
  }
  ...
  if (rename(log_filename.c_str(), GetLogFileName(my_storage->thread_id).c_str())) {
    SPDLOG_ERROR("Durability Error: fail to truncate logfile. errno: {1}", errno);
    exit(1);
  }
  my_storage->truncated_epoch = checkpoint_completed_epoch;
  my_storage->log_file = std::fstream(GetLogFileName(my_storage->thread_id),
                                      std::fstream::out | std::fstream::binary | std::fstream::ate);
}

EpochNumber ThreadLocalLogger::GetMinDurableEpochForAllThreads() {
  EpochNumber min_flushed_epoch = EpochFramework::THREAD_OFFLINE;
  thread_key_storage_.ForEach([&](const ThreadLocalStorageNode* thread_local_node) {
    const EpochNumber epoch = thread_local_node->durable_epoch.load();
    if (epoch == EpochFramework::THREAD_OFFLINE)
      return;
    if (epoch < min_flushed_epoch)
      min_flushed_epoch = epoch;
  });
