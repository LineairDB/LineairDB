...
struct Snapshot {
  ...
  Snapshot(const std::string_view k, const std::byte v[], const size_t s,
           DataItem* const i, std::string_view tn, std::string_view in, const TransactionId ver = 0)
      : key(k), index_cache(i), is_read_modify_write(false), table_name(tn), index_name(in) {
