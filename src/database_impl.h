...
public:
  ...
  Impl(const Config& c = Config())
      : config_(c), thread_pool_(c.max_thread), logger_(config_), callback_manager_(config_),
        epoch_framework_(c.epoch_duration_ms, EventsOnEpochIsUpdated()),
        index_(epoch_framework_, config_), checkpoint_manager_(config_, index_, epoch_framework_) {
    if (Database::Impl::CurrentDBInstance == nullptr) {
      ...
      SPDLOG_ERROR("It is prohibited to allocate two LineairDB::Database instance at "
                   "the same time.");
      exit(1);
    }
  ...
  ~Impl() {
    ...
    SPDLOG_DEBUG("Epoch number and Durable epoch number are ended at {0}, and {1}, "
                 "respectively.",
  ...
  void ExecuteTransaction(ProcedureType proc, CallbackType clbk,
    ...
    for (;;) {
      bool success = thread_pool_.Enqueue(
          [&, transaction_procedure = proc, callback = clbk, precommit_clbk = prclbk]() {
    ...
    return [&](EpochNumber old_epoch) {
      ...
      if (config_.enable_logging) {
        ...
        thread_pool_.EnqueueForAllThreads(
            [&, durable_epoch] { callback_manager_.ExecuteCallbacks(durable_epoch); });
      }
      ...
      if (config_.enable_checkpointing) {
        ...
        thread_pool_.EnqueueForAllThreads(
            [&, checkpoint_completed]() { logger_.TruncateLogs(checkpoint_completed); });
      }
    };
  }
...
private:
  void Recovery() {
    ...
    [[maybe_unused]] auto enqueued =
        thread_pool_.EnqueueForAllThreads([&]() { logger_.RememberMe(durable_epoch); });
    assert(enqueued);
