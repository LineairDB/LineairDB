...
TEST_F(DatabaseTest, ExecuteTransactionWithTemplates) {
  int value_of_alice = 1;
  TestHelper::DoTransactions(
      db_.get(), {[&](LineairDB::Transaction& tx) { tx.Write<int>("alice", value_of_alice); },
                  [&](LineairDB::Transaction& tx) {
...
TEST_F(DatabaseTest, LargeSizeBuffer) {
  ...
  TestHelper::DoTransactions(
      db_.get(), {[&](LineairDB::Transaction& tx) { tx.Write("alice", &alice[0], Size); },
                  [&](LineairDB::Transaction& tx) {
...
TEST_F(DatabaseTest, Scan) {
  ...
  TestHelper::DoTransactions(
      ...
      {[&](LineairDB::Transaction& tx) {
         // Scan
         auto count = tx.Scan<decltype(alice)>("alice", "carol", [&](auto key, auto value) {
           if (key == "alice") {
...
TEST_F(DatabaseTest, SaveAsString) {
  TestHelper::DoTransactions(
      db_.get(), {[&](LineairDB::Transaction& tx) { tx.Write<std::string_view>("alice", "value"); },
                  [&](LineairDB::Transaction& tx) {
...
TEST_F(DatabaseTest, ThreadSafetyInsertions) {
  ...
  ASSERT_NO_THROW({
    TestHelper::DoTransactionsOnMultiThreads(
        db_.get(), {insertTenTimes, insertTenTimes, insertTenTimes, insertTenTimes});
  });
  db_->Fence();
